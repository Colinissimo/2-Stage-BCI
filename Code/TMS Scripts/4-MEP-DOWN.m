...
% set EEG snippet to void in this non-eeg version
EEG_Sess = [];
...
dataFilename = [subDir filesep 'sub-' num2str(subNum) '-up-nmeps' num2str(nMEPs) '-' SesNum '-' blockNumber_str '-tmsnf.dat'];
...
uiwait(msgbox({'This is the TMS-Script'; 'Enable Computer Sounds'}));
    ...
    while exitTrial == false
        ...
        elseif trialState  == 3   && triggerSent==1 && vbl - emgStartTime > Trigger_time - emgStartTime + 1
            ...
            if TMS_State_selector(ii)==2
                ...
                save([subDir filesep 'sub-' num2str(subNum) '-down-' SesNum '-' blockNumber_str '-TMStime-' TMS_time '-mep' (num2str(ii)) '.mat'], 'feedbackData');
            ...
            elseif TMS_State_selector(ii)==1
                save([subDir filesep 'sub-' num2str(subNum) '-down-' SesNum '-' blockNumber_str '-mep' (num2str(ii)) '.mat'], 'feedbackData');
...
save([subDir filesep 'sub-' num2str(subNum) '-down-' SesNum '-' blockNumber_str '-success-' (num2str(nSuccess)) '.mat'], 'nSuccess', 'percentage_success');
...
file_ID = fopen([subDir filesep 'sub-' num2str(subNum) '-down-' SesNum '-' blockNumber_str '-profile.txt'], 'a');
